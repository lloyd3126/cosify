...
export class CreditService {
    ...
    private consumeCreditsMock(userId: string, amount: number): CreditConsumptionResult {
        ...
        const dailyLimitResult = this.checkDailyLimitMock(userId, amount)
        if (!dailyLimitResult.canConsume) {
            return {
                success: false,
                error: CREDIT_ERROR_CODES.DAILY_LIMIT_EXCEEDED,
                dailyUsed: dailyLimitResult.dailyUsed,
                dailyLimit: dailyLimitResult.dailyLimit,
                dailyRemaining: dailyLimitResult.dailyRemaining
            }
        ...
        for (const credit of validCredits) {
            ...
            if (amountToUse === credit.amount) {
                ...
                // Partial consumption - reduce the amount but keep the credit active
        ...
        // Update daily usage in mock data
        const today = this.getTaiwanDate()
        let todayUsage = mockData.dailyUsage.find((usage: any) =>
            usage.userId === userId &&
        ...
        if (!todayUsage) {
            ...
            mockData.dailyUsage.push(todayUsage)
        }

        todayUsage.creditsConsumed += amount
        todayUsage.updatedAt = new Date()

        return {
            success: true,
            consumed: amount,
            transactions,
            newDailyTotal: todayUsage.creditsConsumed
        }
    }
